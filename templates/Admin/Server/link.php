...
<section class="content">
                ...
                <div class="card-body">
                    ...
                    <form action="<?= Router::url(['controller' => 'server', 'action' => 'config', 'admin' => true]) ?>"
                        ...
                        <div class="form-group">
                            <label><?= __('SERVER__TIMEOUT') ?></label>
                        ...
                        <a href="<?= Router::url(['action' => 'switchState', 'admin' => true]) ?>"
                           class="btn btn-<?= ($isEnabled) ? 'danger' : 'success' ?>"><?= ($isEnabled) ? __('SERVER__DISABLE_SERVER') : __('SERVER__ENABLE_SERVER') ?></a>
                        <a href="<?= Router::url(['action' => 'switchCacheState', 'admin' => true]) ?>"
                           class="btn btn-<?= ($isCacheEnabled) ? 'danger' : 'success' ?>"><?= ($isCacheEnabled) ? __('SERVER__DISABLE_CACHE') : __('SERVER__ENABLE_CACHE') ?></a>
                ...
                <div class="card-body">
                    ...
                    <form action="<?= Router::url(['controller' => 'server', 'action' => 'editBannerMsg', 'admin' => true]) ?>"
                        ...
                        <div class="form-group">
                            ...
                            <small><?= __('CONFIG__LANG_AVAILABLE_VARIABLES') ?> : {ONLINE},
                                {ONLINE_LIMIT}</small>
                        </div>
                            ...
                            <form action="<?= Router::url(['controller' => 'server', 'action' => 'link_ajax', 'admin' => true]) ?>"
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="type">
                                        <option value="0"<?= ($value['type'] == '0') ? ' selected' : '' ?>><?= __('SERVER__TYPE_DEFAULT') ?></option>
                                        <option value="1"<?= ($value['type'] == '1') ? ' selected' : '' ?>><?= __('SERVER__TYPE_QUERY') ?></option>
                                        <option value="2"<?= ($value['type'] == '2') ? ' selected' : '' ?>><?= __('SERVER__TYPE_RCON') ?></option>
                                        <option value="3"<?= ($value['type'] == '3') ? ' selected' : '' ?>><?= __('SERVER__TYPE_QUERY_MCPE') ?></option>
                                    </select>
                                </div>
                                ...
                                <button type="submit"
                                        class="btn btn-success"><?= __('GLOBAL__SUBMIT') ?></button>
                                <a href="<?= Router::url(['controller' => 'server', 'action' => 'delete', 'admin' => true, $value['id']]) ?>"
                                   type="submit" class="btn btn-danger"><?= __('GLOBAL__DELETE') ?></a>

                                <button class="btn switchBanner float-right <?=  (isset($value['activeInBanner']) && $value['activeInBanner']) ? 'btn-danger' : 'btn-info' ?>"
                                        id="<?= $value['id'] ?>"><?= (isset($value['activeInBanner']) && $value['activeInBanner']) ? __('SERVER__HIDE_BANNER') : __('SERVER__AFFICH_BANNER') ?></button>
    ...
    <div class="btn btn-success btn-block mb-3" id="add_server"><?= __('SERVER__ADD') ?></div>
</section>
<script>
    ...
    $('.switchBanner').click(function (e) {
        ...
        if (btn.hasClass('btn-danger')) {
            ...
            btn.text("<?= __('SERVER__AFFICH_BANNER') ?>");
        } else {
            btn.removeClass('btn-info');
            btn.addClass('btn-danger');
            btn.text("<?= __('SERVER__HIDE_BANNER') ?>");
        }
    ...
    function selectInfos(select, init) {
        ...
        if (type == 0) {
            var infos = '<div class="alert alert-info"><?= addslashes(__('SERVER__TYPE_DEFAULT_INFOS')) ?></div>'
            ...
            var infos = '<div class="alert alert-info"><?= addslashes(__('SERVER__TYPE_QUERY_INFOS')) ?></div>'
            select.parent().parent().find('input[name="server_data[rcon_port]"]').parent().remove()
            select.parent().parent().find('input[name="server_data[rcon_password]"]').parent().remove()
        } else if (type == 2) {
            var infos = '<div class="alert alert-info"><?= addslashes(__('SERVER__TYPE_RCON_INFOS')) ?></div>'
            var new_server = '<div class="form-group">';
            new_server += '<label><?= __('SERVER__RCON_PORT') ?></label>';
            ...
            new_server += '<label><?= __('SERVER__RCON_PASSWORD') ?></label>';
    ...
    $("#add_server").click(function () {
        ...
        new_server += '<h3 class="card-title"><?= __('SERVER__LINK') ?></h3>';
        new_server += '</div>';
        ...
        new_server += '<div class="form-group">';
        new_server += '<label><?= __('SERVER__TYPE') ?></label>';
        new_server += '<select class="form-control" name="type">';
        new_server += '<option value="0"><?= __('SERVER__TYPE_DEFAULT') ?></option>';
        new_server += '<option value="1"><?= __('SERVER__TYPE_QUERY') ?></option>';
        new_server += '<option value="2"><?= __('SERVER__TYPE_RCON') ?></option>';
        new_server += '<option value="3"><?= __('SERVER__TYPE_QUERY_MCPE') ?></option>';
        new_server += '</select>';
        new_server += '</div>';
        new_server += '<div class="form-group">';
        new_server += '<label><?= __('GLOBAL__NAME') ?></label>';
        new_server += '<input type="text" class="form-control" name="name" placeholder="Ex: MineWeb">';
        new_server += '</div>';
        new_server += '<div class="form-group">';
        new_server += '<label><?= __('SERVER__HOST') ?></label>';
        new_server += '<input type="text" class="form-control" name="host" placeholder="Ex: 127.0.0.1">';
        new_server += '</div>';
        new_server += '<div class="form-group">';
        new_server += '<label><?= __('SERVER__PORT') ?></label>';
        new_server += '<input type="text" class="form-control" name="port" placeholder="Ex: 25565">';
        new_server += '</div>';
        new_server += '<button type="submit" class="btn btn-success"><?= __('GLOBAL__SUBMIT') ?></button>';
        new_server += '</form>';
        new_server += '</div>';
        new_server += '</div>';
        new_server += '</div>';
