                                    ...
                                    <td>
                                        ...
                                        if ($lastVersion && $value->version != $lastVersion) { ?>
                                            <a <?= (explode('.', $lastVersion)[0] > explode('.', $value->version)[0] ? 'data-warning-update' : '') ?>
                                                    ...
                                                    class="btn btn-warning update"><?= $Lang->get('GLOBAL__UPDATE') ?></a> <!-- ICI -->
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
                ...
                <div class="card-body">
                    ...
                    <?php } else { ?>
                        <div class="alert alert-danger"><b><?= $Lang->get('GLOBAL__ERROR') ?>
                                : </b><?= $Lang->get('PLUGIN__NONE_AVAILABLE') ?></div>
                    <?php } ?>
            ...
            $.get('<?= Router::url(['action' => 'install', 'admin' => true]) ?>/' + slug, function (data) {
                ...
                if (data !== false) {
                    ...
                    if (data.statut == "success") {
                        ...
                        tr += '<td>';
                        tr += '<a href="<?= Router::url(['action' => 'disable', 'admin' => true]) ?>/' + data.plugin.DBid + '" class="btn btn-info"><?= $Lang->get('GLOBAL__DISABLED') ?></a>';
                        tr += "\n";
                        tr += '<a onClick="confirmDel(\'<?= Router::url(['controller' => 'plugin', 'action' => 'delete', 'admin' => true]) ?>/' + data.plugin.DBid + '\')" class="btn btn-danger"><?= $Lang->get('GLOBAL__DELETE') ?></a>';
                        tr += '</td>';
                        tr += '</tr>';
