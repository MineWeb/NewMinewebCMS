        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header with-border">
                    <h3 class="card-title"><?= $Lang->get('PERMISSIONS__LABEL') ?></h3>
                </div>
                <div class="card-body">

                    <button data-toggle="modal" data-target="#addRank"
                            class="btn btn-block btn-success"><?= $Lang->get('USER__RANK_ADD') ?></button>
                                    ...
                                    <?php if (!empty($all_ranks)) { ?>
                                        <?php foreach ($all_ranks as $k => $data) { ?>
                                            <td><input type="checkbox"
                                                       name="<?= $permission ?>-<?= $data['rank_id'] ?>"<?= (isset($ranks[$data['rank_id']]) && $ranks[$data['rank_id']]) ? ' checked="checked"' : '' ?>>
                                            </td>
                                        <?php } ?>
                            ...
                            <tr>
                                ...
                                if (!empty($all_ranks)) {
                                    foreach ($all_ranks as $k => $data) {
                                        ...
                                        echo '<td><a class="btn btn-danger" href="' . Router::url(['controller' => 'permissions', 'action' => 'delete_rank', 'admin' => true, $data['rank_id']]) . '">' . $Lang->get('GLOBAL__DELETE') . '</a></td>';
                                    }
                                }
                                ?>
                            </tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= Router::url(['action' => 'add_rank', 'admin' => true]) ?>" method="post" data-ajax="true"
                      data-redirect-url="#">
