...
$databasePath = ROOT . DS . 'config' . DS . 'databases.json';
$database = json_decode(file_get_contents($databasePath));
$needDisplayDatabase = str_contains($database->username, "LOGIN1");
            ...
            if ($sql_type == 0) { //mysql
                ...
                if (!empty($_POST['host']) && !empty($_POST['database']) && !empty($_POST['login'])) {
                    ...
                    if (!$sql_error) {
                        ...
                        $database->username = $sql_user;
                        $database->password = $sql_pass;
                        $database->database = $sql_name;

                        echo editDatabaseFile($databasePath, $database);
                        exit;
                    } else {
                ...
                $database->database = "/src/Data/database.db";
...
function editDatabaseFile($dataPath, $content)
{
    if (!file_exists($dataPath) || !is_writable($dataPath))
        return json_encode(['status' => false, 'msg' => 'Le fichier /config/databases.json ne peut pas être écris !']);

    file_put_contents($dataPath, json_encode($content));
...
$compatible['chmod'] = is_writable(ROOT . DS . 'config') && is_writable(ROOT . DS . 'src' . DS . 'Data') && is_writable(ROOT . DS . 'plugins') && is_writable(ROOT . DS . 'tmp') && is_writable(ROOT . DS . 'webroot' . DS . 'js');
...
if (!$compatible['chmod']) {
    ...
    if (!is_writable(ROOT . DS . 'src' . DS . 'Data')) {
        $help['chmod'] = "Le dossier /src/Data ne peut être écrit. <br /><br />";
    }

    if (!is_writable(ROOT . DS . 'plugins')) {
        $help['chmod'] = "Le dossier /plugins ne peut être écrit. <br /><br />";
    }
    ...
    if (!is_writable(ROOT . DS . 'tmp')) {
        $help['chmod'] = "Le dossier /tmp ne peut être écrit. <br /><br />";
    }

    if (!is_writable(ROOT . DS . 'webroot' . DS . 'js')) {
        $help['chmod'] = "Le dossier /webroot/js ne peut être écrit. <br /><br />";
    }
}
