...
class NotificationTable extends Table
{
    ...
    public function getFromUser($user_id, $type)
    {
        ...
        \Cake\I18n\DateTime::$wordFormat = 'd/m/y';

        foreach ($query as $notification) {
            if ($notification['from'] == null) {
                $from = null;
            } else {
                $from = $UserModel->getFromUser('pseudo', $notification['from']);
            }
            ...
            $data[] = [
                ...
                'time' => \Cake\I18n\DateTime::parse($notification['created'])->timeAgoInWords(),
    ...
    */
    ...
    public function setToRank($content, $rank_id, $from = null, $type = 'user', $group = null)
    {
        if (!$group)
            $group = $this->generateGroup();
        $UserModel = TableRegistry::getTableLocator()->get('User');
        $usersToNotify = $UserModel->find('all', conditions: ['rank' => $rank_id])->all();

        foreach ($usersToNotify as $user) {
            $this->setToUser($content, $user['id'], $from, $type, $group);
        }
    ...
    public function setToAll($content, $from = null)
    {
        ...
        $this->getConnection()->execute("INSERT INTO notifications (`group`, `user_id`, `from`, `content`, `type`, `created`) SELECT '$group', id, $from, '$content', 'user', '" . date('Y-m-d H:i:s') . "' FROM users");
    }
