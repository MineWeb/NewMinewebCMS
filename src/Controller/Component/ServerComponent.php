...
class ServerComponent extends Component
{
    ...
    {
        $get = TableRegistry::getTableLocator()->get("Server")->find()->first();
        return (!empty($get)) ? $get['id'] : null;
    }
    ...
    public function getServerConfig($server_id = false)
    {
        if ($server_id === false)
            $server_id = $this->getFirstServerID();
        if (!$server_id)
        ...
        if ($configuration['server_state'] != 1)
            return $this->config[$server_id] = false;
        ...
        return $this->config[$server_id] = [
            ...
            'port' => $search['port'],
            'type' => $search['type'],
            'data' => json_decode($search['data'], true)
        ];
    }
    ...
    public function rcon($config = false, $cmd = '')
    {
        ...
        $rcon = new Rcon($config['ip'], $config['port'], $config['password'], $this->getTimeout());
        if ($rcon->connect())
    ...
    private function getTimeout()
    {
        if (!empty($this->timeout))
            return $this->timeout;
        return $this->configModel->find()->first()['server_timeout'];
    }
    ...
    {
        if (!isset($this->key))
            $this->key = $this->configModel->find()->first()['server_secretkey'];
    ...
    {
        if (!isset($this->key))
            $this->key = $this->configModel->find()['server_secretkey'];
        $iv = base64_decode($iv);
    ...
    public function userIsConnected($username, $server_id = false)
    {
        ...
        if ($result && isset($result['IS_CONNECTED']) && $result['IS_CONNECTED'] || $this->getServerConfig($server_id)['type'] == 2)
            return true;
        return false;
    }
    ...
    public function online($server_id = false, $debug = false)
    {
        if (!$server_id) // first server config
            $server_id = $this->getFirstServerID();
        if ($this->configModel->find()->first()['server_state'] == '0') // enabled
    ...
    public function getSecretKey()
    {
        ...
        // save
        $config = $this->configModel->get($config['id']);
        $config->set(['server_secretkey' => $key]);
        $this->configModel->save($config);
        return $key;
    }
