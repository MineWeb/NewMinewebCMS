...
class ThemeComponent extends Component
{
    ...
    {
        $this->themesFolder = ROOT . DS . 'templates' . DS . 'Themed';
    ...
    public function getThemeConfig($slug, $array = false)
    {
    ...
    {
        $config = $this->getThemeConfig($slug); // on récup la config
    ...
    public function install($slug, $update = false)
    {
        ...
        // Check macosx folder
        if (file_exists($this->themesFolder . DS . '__MACOSX'))
            rmdir($this->themesFolder . DS . '__MACOSX');
    ...
    private function download($slug)
    {
        ...
        // Temporary file
        $zipFile = ROOT . DS . 'tmp' . DS . 'theme-' . $slug . '.zip';
        ...
        if (!file_exists(ROOT . DS . 'templates' . DS . 'Themed' . DS . $slug) && !mkdir(ROOT . DS . 'templates' . DS . 'Themed' . DS . $slug))
            return 'THEME__ERROR_INSTALL_UNZIP';
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            $dest = ROOT . DS . 'templates' . DS . 'Themed' . DS . $slug . substr($filename, strlen('Theme-' . $slug . '-master'));
    ...
    public function setThemeConfig($slug, $config = [])
    {
        $path = $this->getPath($slug) . DS . 'Config' . DS . 'config.json';
        if (strtolower($slug) === 'default' || !file_exists($path))
    ...
    public function processCustomData($slug, ServerRequest $request)
    {
        ...
        if ($request->getData('img_edit')) {
            ...
            if ($checkIfImageAlreadyUploaded) {
                $request = $request->withData('logo', Router::url('/') . 'img' . DS . 'uploads' . $request->getData('img-uploaded'));
                ...
                if ($infos) {
                    ...
                    $request->withData('logo', Router::url('/') . 'img' . DS . 'uploads' . DS . 'theme_logo.' . $infos['extension']);
                } else {
                    $request->withData('logo', false);
                }

            }
        } else {
            $request->withData('logo', $config->logo);
        }
