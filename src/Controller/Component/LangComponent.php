    ...
    public function getLang($mode = false, $language = null)
    {
        ...
        if (isset($plugins) && !empty($plugins)) {
            foreach ($plugins as $key => $value) {
                $name = $value->slug;
                $language_file = $this->getLangFile($this->EyPlugin->pluginsFolder . DS . $name, $language['path']);
                if (isset($language["messages"]) && is_array($language['messages']) && isset($language_file) && is_array($language_file)) {
                    $language['messages'] = array_merge($language['messages'], $language_file);
                }
            }
        ...
        if (isset($themes) && !empty($themes)) {
            foreach ($themes as $key => $value) {
                $name = $value->slug;
                $language_file = $this->getLangFile($this->Theme->themesFolder . DS . $name, $language['path']);
                if (isset($language["messages"]) && is_array($language['messages']) && isset($language_file) && is_array($language_file)) {
                    $language['messages'] = array_merge($language['messages'], $language_file);
                }
            }
    ...
    public function update($JSON, $file)
    {
        ...
        if (!file_exists($file)) {
            ...
            foreach ($fileContent['MESSAGES'] as $key => $value) { // on parcours les messages pour éventuellement les mettre à jours
                ...
                if (file_exists(ROOT . DS . 'tmp' . DS . 'logs' . DS . 'update' . DS . 'lang' . DS . $path . '.log.json')) {
                    $log = file_get_contents(ROOT . DS . 'tmp' . DS . 'logs' . DS . 'update' . DS . 'lang' . DS . $path . '.log.json');
                    $log = json_decode($log, true);
                } else {
                    $log['update'] = [];
                }
