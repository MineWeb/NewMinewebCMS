...
class UpdateComponent extends Component
{
    ...
    private function check()
    {
        ...
        $this->lastVersion = trim($remoteVersion ?? file_get_contents($this->updateCacheFile));
    ...
    public function updateCMS($componentUpdated = false)
    {
        ...
        if (!$componentUpdated) {
            ...
            $zip = new ZipArchive;
            if ($zip->open(ROOT . DS . 'tmp' . DS . $this->lastVersion . '.zip') !== true) return false;

            $path = ROOT . DS . 'src' . DS . 'Controller' . DS . 'Component' . DS . 'UpdateComponent.php';
        ...
        if (method_exists($this->Util, 'saveFolderInZIP'))
            $this->Util->saveFolderInZIP(ROOT . DS, ROOT . DS . 'tmp' . DS, "SAVE-" . $this->cmsVersion . "-" . time());
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if ($stats['size'] > 0) {
                ...
                $path = "zip://" . ROOT . DS . "update_test" . DS . "tmp" . DS . $this->lastVersion . ".zip#{$this->source['repo']}-{$this->lastVersion}/" . "$filename";
        ...
        // Remove zip
        @unlink(ROOT . DS . 'tmp' . DS . $this->lastVersion . '.zip');
    ...
    private function downloadUpdate()
    {
        ...
        if (!($filesContent = $this->controller->sendGetRequest("https://github.com/{$this->source['owner']}/{$this->source['repo']}/archive/v{$this->lastVersion}.zip")))
            return false;
        $write = fopen(ROOT . DS . 'tmp' . DS . $this->lastVersion . '.zip', 'w+');
