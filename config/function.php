...
function findRecursive($path, $extensions = array()): array
{
    $result = array();

    $dir = new RecursiveDirectoryIterator($path);
    $filter = new RecursiveCallbackFilterIterator($dir, function (SplFileInfo $current) use ($extensions) {
        if ($current->getFilename()[0] === "." || $current->getFilename()[0] === "..")
            return false;

        $ext = $current->getExtension();
        if ($current->isDir() || in_array($ext, $extensions))
            return true;
    ...
    foreach ($iterator as $file) {
        if ($file->isDir()) {
            $result = array_merge($result, findRecursive($file->getPathname()));
            continue;
        }
